...
class GecBERTModel(object):
    def __init__(self,
        ...
        self.common_set = ['的', '了', '我', '你', '他', '着', '得', '地', '这', '那', '里', '会', '个', '还',
                           '只', '而', '是', '还', '会', '给']
        self.mask_bert = MaskedBert.from_pretrained(path='./plm/chinese_bert_wwm_ext_pytorch',
                                                    device=self.device,
                                                    sentence_length=256)
    ...
    def get_token_action(self, index, prob, sugg_token, iter_num, length):
        ...
        if (prob < self.min_probability and not sugg_token.startswith('$MOVE')) or sugg_token in [UNK, PAD, '$KEEP']:
            return None
        # if iter_num < 2 and sugg_token.startswith('$MOVE'):
        #     return None
        # if sugg_token.startswith('$REPLACE_') and prob < self.min_replace_prob:
        #     return None
        # if sugg_token.startswith('$APPEND_') and prob < self.min_append_prob:
        #     return None
        # if sugg_token.startswith('$DELETE') and prob < self.min_delete_prob:
        #     return None
        # if sugg_token in self.append_noum and prob < self.min_append_special:
        #     return None
        #if sugg_token in ['$APPEND_我', '$APPEND_他'] and index < 2 and prob < 0.9:
        #    return None
        #if sugg_token.startswith('$DELETE') and iter_num == 0 and prob < 0.6:
        #    return None
        #if iter_num == 0 and (sugg_token.startswith('$REPLACE') or sugg_token.startswith('$APPEND')) \
        #        and sugg_token.split('_')[1] in self.common_set and prob < 0.51:
        #    return None
        if iter_num == 0 and prob < 0.6:
            return None
        #
        #if sugg_token in ['$APPEND_我', '$APPEND_的', '$APPEND_了', '$APPEND_地'] and prob < 0.8:
        #    return None
        #if (sugg_token.startswith('$APPEND') or sugg_token.startswith('$REPLACE')) and \
        #        sugg_token.split('_')[1] in self.tail_noum and prob < 0.95:
        #    return None
    ...
    def postprocess_batch(self, batch, all_probabilities, all_idxs,
        ...
        for tokens, probabilities, idxs, error_prob, d_tags_idxs, correct_probs in zip(batch,
            ...
            for i in range(length + 1):
                ...
                print('candidate', candidate_sugg, can_prob)
        ...
        while idx < len(edits):
            if edits[idx][2] and edits[idx][2][-1].isdigit():
                ...
                if len(move_span) == 1:
                    ...
                    offset = [int(x[2]) for x in move_span]
                    if sum(offset) == 0:
                        post_edit.extend(move_span)
                    else:
                        result = self.dilate_move_prob(tokens, correct_probs, move_span)
                        if result is not None:
                            add_flag = True
                            for _result in result:
                                if abs(int(_result[2])) > len(result):
                                    add_flag = False
                                    break
                            if add_flag:
    ...
    def dilate_move_prob(self, tokens, probs, move_span):
        ...
        start_move = move_span[0][2]
        _path_idx = 0
        if '$MOVE_' + str(start_move) not in top_path[1]:
            return None
        if '$MOVE_' + str(start_move) in top_path[0]:
            _path_idx = 0
        else:
            _path_idx = 1
        ...
        for idx in range(len(top_path)):
            ...
            for _path, _prob in zip(cur_path, cur_prob):
                for value, prob in zip(tmp_path, tmp_prob):
                    if 0 in value[1:]:
                        first_zero = value[1:].index(0) + 1
                    if 0 not in value and len(set(value)) > 2:
                        continue
                    if value and value[0] == 0 and len(value) > 1 and 0 not in value[1:] \
                            and len(set(value[1:])) > 2:
                        continue
                    if 0 in value[1:] and sum(value[first_zero:]) != 0:
                        continue
        ...
        for _idx, _sum in enumerate(sum_all):
            ...
            if len(set(_path[pos_sidx: pos_eidx])) != 1:
                continue
            if _path_idx == 0 and candidate_path[_idx][0] == int(start_move):
                prefix = _path[_path_idx: _path_idx+2]
            else:
                prefix = [candidate_path[_idx][_path_idx]]
            if len(set(_path)) == 2 and len(set(prefix)) == 1:
        ...
        print('ppl_score', ppl_score, min_idx)
        if min_idx == 0:
            return None
