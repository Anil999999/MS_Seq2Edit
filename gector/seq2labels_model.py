...
class Seq2Labels(Model):
    def __init__(self, vocab: Vocabulary,
        ...
        self.weights = []
        if weights_path:
            self.weights = [int(line.strip()) for line in open(weights_path, 'r', encoding='utf-8')]
    ...
    def decode(self, encoded_text: torch.LongTensor = None,
        ...
        if self.confidence > 0:  # 给$KEEP标签添加一个置信度bias，优先预测$KEEP，防止模型过多地纠错，属于一个小trick
            ...
            # probability_change = [self.confidence] + [0] + [self.confidence] * 16 + [0] * (self.num_labels_classes - 18)
