...
def align_sequences(args):
    ...
    while idx < len(diffs):
        ...
        if tag == 'equal':
            idx += 1
            continue
        elif tag == 'delete' and idx + 2 < len(diffs) and diffs[idx+1][0] == 'equal' and diffs[idx+2][0] == 'insert':
            # delete all words separatly 6 7 6 6 | 7 8 6 7 | 8 8 7 8
            # 乱序的情况尝试利用move操作来解决，减少append时的操作数，同时修正模型的delete概率高的情况
            next_tag, next_i1, next_i2, next_j1, next_j2 = diffs[idx+1]
            last_tag, last_i1, last_i2, last_j1, last_j2 = diffs[idx+2]
            next_source = _split(" ".join(source_tokens[next_i1: next_i2]))
            last_target = _split(" ".join(target_tokens[last_j1: last_j2]))
            if last_target == source_part and (last_i1 - next_i1) < 13:
                for j in range(len(source_part)):
                    edit = [(i1 + j, i1 + j + 1), '$MOVE_%s' % (str(len(next_source)))]
                    all_edits.append(edit)
                for j in range(len(next_source)):
                    edit = [(next_i1 + j, next_i1 + j + 1), '$MOVE_%s' % (str(-len(source_part)))]
                    all_edits.append(edit)
                idx += 3
            else:
            ...
            for j in range(i2 - i1):
                ...
                all_edits.append(edit)
            idx += 1
        elif tag == 'insert' and idx + 2 < len(diffs) and diffs[idx + 1][0] == 'equal' and diffs[idx + 2][0] == 'delete':
            next_tag, next_i1, next_i2, next_j1, next_j2 = diffs[idx+1]
            last_tag, last_i1, last_i2, last_j1, last_j2 = diffs[idx+2]
            next_source = _split(" ".join(source_tokens[next_i1: next_i2]))
            last_target = _split(" ".join(source_tokens[last_i1: last_i2]))
            if last_target == target_part and (last_i1 - next_i1) < 13:
                for j in range(len(next_source)):
                    edit = [(i1 + j, i1 + j + 1), '$MOVE_%s' % (str(len(target_part)))]
                    all_edits.append(edit)
                for j in range(len(target_part)):
                    edit = [(last_i1 + j, last_i1 + j + 1), '$MOVE_%s' % (str(-len(next_source)))]
                    all_edits.append(edit)
                idx += 3
            else:
...
def convert_data_from_raw_files(source_file, target_file, output_file, vocab_path, min_count, save_vocab=False, worker_num=8):
    ...
    print('the new label size is {}'.format(len(labels)))
    if save_vocab:
    ...
    for level in range(max_level):
        ...
        for edits in relevant_edits:
            ...
            if label == "$DELETE":
                ...
                del origin_targets[target_pos]
                shift_idx -= 1
            elif label.startswith("$APPEND_"):
                word = label.replace("$APPEND_", "")
                target_tokens[target_pos + 1: target_pos + 1] = [word]
                origin_targets[target_pos + 1: target_pos + 1] = [word]
                shift_idx += 1
            elif label.startswith("$REPLACE_"):
                word = label.replace("$REPLACE_", "")
                target_tokens[target_pos] = word
                origin_targets[target_pos] = word
            elif label.startswith('$MOVE_'):
                edit_offset = int(label.replace('$MOVE_', ''))
                target_tokens[target_pos + edit_offset: target_pos + edit_offset + 1] = origin_targets[target_pos: target_pos+1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--source',
                        ...
                        default='../data/csc.output',
                        type=str)  # GEC数据源端文件（病句，每行一句），分字
    parser.add_argument('-t', '--target',
                        help='Path to the target file',
                        default='../data/target.txt',
                        type=str)  # GEC数据目标端文件（正确句子，每行一句，和source对应），分字
    parser.add_argument('-o', '--output_file',
                        help='Path to the output file',
                        default='../data/test.output',
                        type=str)  # 输出结果，可直接用于GECToR训练
