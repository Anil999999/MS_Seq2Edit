...
def get_target_sent_by_edits(source_tokens, edits):
    ...
    for edit in edits:
        ...
        if label == "":
            if target_tokens:
                del origin_target[target_pos]
                del target_tokens[target_pos]
                shift_idx -= 1
        elif label[-1].isdigit():
            target_tokens[target_pos + int(label): target_pos + int(label) + 1] = origin_target[target_pos: target_pos + 1]
        elif start == end:
            word = label.replace("$APPEND_", "")  # 添加操作
            if word in target_tokens[target_pos - 1: target_pos + 1]:
                continue
            target_tokens[target_pos: target_pos] = [word]
            origin_target[target_pos: target_pos] = [word]
            shift_idx += 1
            ...
            origin_target[target_pos + 1: target_pos + 1] = [label]
            shift_idx += 1
